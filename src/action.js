...
async function announceNewReleasePR(config, prNumber) {
  ...
  console.log(
    "changeset.addTicketDetailsToChanges([" + changeList.length + "])"
  );
  ...
  await github.patch(config, new URL("pulls/" + prNumber, config.apiUrl), {
    body: changelog,
  });
}

function prepareSlackMessage(config, pr, changes) {
  const message = new slack.SlackMessage(
    config.slackToken,
    `New ${config.deployee} Release proposed in PR #${pr.number}`
  );

  message.addBlocks(
    new slack.TextBlock(
      `A new *${config.deployee}* production release has been proposed in PR #${pr.number}`
    ),
    ...
    new slack.TextBlock(null)
      ...
      .addField(
        changes.map((change) => change.commits).reduce((a, v) => a + v, 0) +
          " Commits"
      ),
    new slack.Divider(),
    ...changes.map((change) =>
      new slack.TextBlock(
        `<${change.link}|${change.ticket}> *[${change.status}]* ${change.summary} ` +
          change.prs.map((pr) => `<${prUrl(config, pr)}|PR #${pr}>`).join(", ")
      ).addButtonAccessory(change.link, "JIRA")
    )
  );
...
async function announceDeploymentStart(config) {
  const message = new slack.SlackMessage(
    config.slackToken,
    `*${config.deployee}*: production deployment started.`
  );
...
async function announceDeploymentComplete(config) {
  const message = new slack.SlackMessage(
    config.slackToken,
    `*${config.deployee}*: production deployment completed.`
  );
